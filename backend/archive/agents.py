...
class VendorAgent:
    ...
    def __init__(self, approved_vendors: Set[str] = None):
        ...
        log.info(
            f"VendorAgent initialized with {len(self.approved_vendors)} approved vendors"
        )
    ...
    def check_vendor(self, invoice: Invoice) -> VendorCheckResult:
        ...
        try:
            ...
            log.info(
                f"Vendor check: '{normalized_vendor}' → {'VALID' if is_valid else 'INVALID'} ({risk_factor} risk)"
            )
            return result
    ...
    def _calculate_similarity(self, vendor1: str, vendor2: str) -> float:
        ...
        char_similarity = 1 - (
            self._levenshtein_distance(v1, v2) / max(len(v1), len(v2))
        )
...
class TotalsAgent:
    ...
    def check_totals(self, invoice: Invoice) -> TotalsCheckResult:
        ...
        try:
            ...
            if not matches:
                ...
                if self._check_common_mistakes(
                    invoice, reported_total, calculated_total
                ):
                    risk_factor = "HIGH"
            ...
            log.info(
                f"Totals check: ${reported_total} vs ${calculated_total} → {status_msg}"
            )
            return result
    ...
    def _check_common_mistakes(
        self, invoice: Invoice, reported: float, calculated: float
    ) -> bool:
    ...
    def get_calculation_stats(self) -> Dict[str, Any]:
        ...
        avg_difference = (
            sum(calc["difference"] for calc in self.calculation_history) / total_calcs
        )
...
class PatternAgent:
    ...
    def __init__(self):
        ...
        log.info(
            f"PatternAgent initialized with {len(self.suspicious_keywords)} suspicious keywords"
        )
    ...
    def analyze_patterns(self, invoice: Invoice) -> PatternAnalysisResult:
        ...
        try:
            ...
            log.info(
                f"Pattern analysis: {anomaly_count} anomalies, {risk_factor} risk, severity {severity_score:.2f}"
            )
            return result
    ...
    def _analyze_item(self, item, item_number: int) -> tuple:
        ...
        for keyword in self.suspicious_keywords:
            if keyword in description:
                anomalies.append(
                    f"Item {item_number}: Suspicious description contains '{keyword}' - {item.description}"
                )
                severity += 0.3
                break
        ...
        if line_total > self.high_value_threshold:
            anomalies.append(
                f"Item {item_number}: High-value item - {item.description} (${line_total:.2f})"
            )
            severity += 0.2
        ...
        if quantity > self.high_quantity_threshold:
            anomalies.append(
                f"Item {item_number}: High quantity - {quantity} of {item.description}"
            )
            severity += 0.2
        ...
        if unit_price > 1000:
            anomalies.append(
                f"Item {item_number}: High unit price - {item.description} at ${unit_price:.2f} each"
            )
            severity += 0.2
        ...
        if quantity <= 0:
            anomalies.append(
                f"Item {item_number}: Invalid quantity - {item.description} has quantity {quantity}"
            )
            severity += 0.4

        if unit_price <= 0:
            anomalies.append(
                f"Item {item_number}: Invalid price - {item.description} has price ${unit_price:.2f}"
            )
            severity += 0.4
        ...
        if unit_price > 0 and unit_price % 1 == 0 and unit_price >= 100:
            ...
            anomalies.append(
                f"Item {item_number}: Suspiciously round high price - {item.description} at ${unit_price:.0f}"
            )
            severity += 0.1
    ...
    def _analyze_invoice_level(self, invoice: Invoice) -> tuple:
        ...
        if len(invoice.items) > 50:
            anomalies.append(
                f"Unusually high number of line items: {len(invoice.items)}"
            )
            severity += 0.2
        ...
        if len(invoice.items) > 3 and round_count / len(invoice.items) > 0.8:
            anomalies.append(
                "High proportion of round-number prices (potential fabrication)"
            )
            severity += 0.2
        ...
        if len(invoice.items) > 5:
            ...
            if prices:
                ...
                if outliers:
                    anomalies.append(
                        f"Extreme price outliers detected: {len(outliers)} items significantly above average"
                    )
                    severity += 0.1
    ...
    def get_pattern_stats(self) -> Dict[str, Any]:
        ...
        avg_anomalies = (
            sum(p["anomalies_found"] for p in self.pattern_history) / total_analyses
        )
...
class AgentCoordinator:
    ...
    def execute_tasks(self, invoice: Invoice, tasks: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "CheckVendor" in tasks:
                try:
                    results["vendor"] = self.vendor_agent.check_vendor(
                        invoice
                    ).to_dict()
                except Exception as e:
            ...
            if "CheckTotals" in tasks:
                try:
                    results["totals"] = self.totals_agent.check_totals(
                        invoice
                    ).to_dict()
                except Exception as e:
            ...
            if "AnalyzePatterns" in tasks:
                try:
                    results["patterns"] = self.pattern_agent.analyze_patterns(
                        invoice
                    ).to_dict()
                except Exception as e:
            ...
            return {
                ...
                "patterns": {
                    "anomalies_found": 1,
                    "risk_factor": "HIGH",
                    "anomaly_details": [f"Execution error: {str(e)}"],
                },
            }
        ...
        self.execution_history.append(
            {
                ...
                "patterns_risk": results.get("patterns", {}).get(
                    "risk_factor", "UNKNOWN"
                ),
    ...
    def _calculate_overall_risk(self, results: Dict[str, Any]) -> str:
        ...
        risk_levels = {"HIGH": 3, "MEDIUM": 2, "LOW": 1, "UNKNOWN": 2}
    ...
    def get_agent_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.execution_history:
                ...
                avg_time = (
                    sum(e["execution_time"] for e in self.execution_history)
                    / total_executions
                )
                high_risk_count = sum(
                    1 for e in self.execution_history if e["overall_risk"] == "HIGH"
                )
            ...
            return {
                ...
                "vendor_agent": {"status": "ready", "stats": vendor_stats},
                "totals_agent": {"status": "ready", "stats": totals_stats},
                "pattern_agent": {"status": "ready", "stats": pattern_stats},
        ...
        try:
            ...
            if "totals" in config:
                ...
                if "tolerance" in totals_config:
                    ...
                    log.info(
                        f"Updated totals tolerance to: {self.totals_agent.tolerance}"
                    )
            ...
            if "patterns" in config:
                ...
                if "high_value_threshold" in patterns_config:
                    self.pattern_agent.high_value_threshold = float(
                        patterns_config["high_value_threshold"]
                    )
                    log.info(
                        f"Updated high value threshold to: {self.pattern_agent.high_value_threshold}"
                    )

                if "high_quantity_threshold" in patterns_config:
                    self.pattern_agent.high_quantity_threshold = float(
                        patterns_config["high_quantity_threshold"]
                    )
                    log.info(
                        f"Updated high quantity threshold to: {self.pattern_agent.high_quantity_threshold}"
                    )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_executions = (
                executions[-20:] if len(executions) >= 20 else executions
            )
            recent_avg_time = sum(e["execution_time"] for e in recent_executions) / len(
                recent_executions
            )
            recent_high_risk = sum(
                1 for e in recent_executions if e["overall_risk"] == "HIGH"
            )
            ...
            for threshold_key in ["high_value_threshold", "high_quantity_threshold"]:
                if threshold_key in patterns_config:
                    try:
                        ...
                        if value < 0:
                            errors.append(
                                f"patterns.{threshold_key} must be non-negative"
                            )
                    except (ValueError, TypeError):
