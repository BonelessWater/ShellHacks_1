"""Minimal shim for backend.archive.main_pipeline used in integration tests.

This file provides tiny placeholder implementations so the integration
script can import the module even if the full implementation isn't present.
It's intentionally minimal — real logic should live in the full codebase.
"""
...
class MainPipeline:
    """Very small stub of a pipeline class expected by callers."""

    def __init__(self, config: Any = None):
        self.config = config

    def run(self, *args, **kwargs) -> bool:
        """Pretend to run the pipeline and return success."""
        return True


def run_pipeline(*args, **kwargs) -> bool:
    """Convenience function wrapper used by some importers."""
    mp = MainPipeline(kwargs.get("config"))
    return mp.run(*args, **kwargs)
#!/usr/bin/env python3
"""
...
from data_models import (DataValidator, FraudDetectionResult,
                         FraudDetectionSummary, Invoice, JSONParser)
...
from backend.llm import get_configured_lm
...
class FraudDetectionPipeline:
    ...
    def _fallback_plan_tasks(
        self, invoice: Invoice, known_error: str = ""
    ) -> List[str]:
    ...
    def _fallback_summarize(
        self, vendor_result: Dict, totals_result: Dict, patterns_result: Dict
        ...
        confidence_score = (
            0.8 if not risk_factors else (0.9 if len(risk_factors) >= 2 else 0.7)
        )
    ...
    def _llm_plan_tasks(
        ...
        try:
            ...
            plan_response = self.dspy_modules["planner"](
                invoice_json=invoice_json, known_error=known_error
            )
    ...
    def _llm_validate_tasks(self, invoice: Invoice, tasks: List[str]) -> List[str]:
        ...
        try:
            ...
            validate_response = self.dspy_modules["validator"](
                invoice_json=invoice_json, proposed_tasks=json.dumps(tasks)
            )
    ...
    ) -> Optional[FraudDetectionSummary]:
    ...
    def _llm_review(
        self, invoice: Invoice, summary: FraudDetectionSummary
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            review_response = self.dspy_modules["reviewer"](
                invoice_json=invoice_json, summary=summary_json
            )
    ...
    def run_detection(
        self, invoice_data: Dict[str, Any], max_iterations: int = 3
            ...
            try:
                ...
                if not summary:
                    summary = self._fallback_summarize(
                        results["vendor"], results["totals"], results["patterns"]
                    )
        ...
        conservative_results = self.agent_coordinator.execute_tasks(
            invoice, conservative_tasks
        )
...
def run_fraud_detection(
    invoice_data: Dict[str, Any], max_iterations: int = 3
) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    test_invoice = {
        ...
        "date": "2024-09-26",
        "items": [
            {"description": "Office supplies", "quantity": 10, "unit_price": 15.50},
            {
                "description": "Gift cards",  # Suspicious
                "quantity": 5,
                "unit_price": 100.00,
            },
            {"description": "Consulting services", "quantity": 20, "unit_price": 75.00},
        ],
