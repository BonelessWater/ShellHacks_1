...
class DatabaseManager:
    ...
    def __init__(
        self, project_id: str, dataset_name: str, table_name: str = "invoices"
    ):
    ...
    def init_database(self):
        ...
        try:
            ...
            print(
                f"Table {self.table_id} not found. Please create it in your BigQuery project."
            )
    ...
    def save_invoice(self, invoice: InvoiceData) -> bool:
        ...
        try:
            ...
            invoice_logger.get_logger("data_extraction").error(
                f"Error saving invoice to BigQuery: {e}"
            )
            return False
    ...
    def find_duplicates(self, invoice: InvoiceData) -> list:
        ...
        try:
            ...
            invoice_logger.get_logger("duplicate_detection").error(
                f"Error finding duplicates in BigQuery: {e}"
            )
            return []
...
class SpecializedAgent:
    ...
    async def process(
        self, invoice: InvoiceData, context: Dict[str, Any] = None
    ) -> VerificationResult:
...
class DataExtractionAgent(SpecializedAgent):
    ...
    def __init__(self):
        ...
        self.vendor_required_fields = ["name", "address", "phone", "email", "tax_id"]
    ...
    async def process(
        ...
        self.logger.info(
            f"Processing data extraction for invoice {invoice.invoice_number}"
        )
        ...
        if missing_fields:
            ...
            recommendations.append(
                f"Obtain missing fields: {', '.join(missing_fields)}"
            )
        ...
        return self._create_result(
            status, confidence, findings, recommendations, processing_time
        )
...
class DuplicateDetectionAgent(SpecializedAgent):
    ...
    async def process(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Processing duplicate detection for invoice {invoice.invoice_number}"
        )
    ...
    def _calculate_similarity(
        self, invoice1: InvoiceData, invoice2: Dict[str, Any]
    ) -> float:
...
class AmountValidationAgent(SpecializedAgent):
    ...
    async def process(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Processing amount validation for invoice {invoice.invoice_number}"
        )
...
class VendorVerificationAgent(SpecializedAgent):
    ...
    async def process(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Processing vendor verification for invoice {invoice.invoice_number}"
        )
        ...
        # Risk assessment
        risk_assessment = self._assess_vendor_risk(
            vendor_name, invoice, history_analysis
        )
        findings.append(
            {
    ...
    def _check_vendor_whitelist(
        self, vendor_name: str, vendor_info: Dict[str, str]
    ) -> List[Dict[str, Any]]:
    ...
    def _validate_vendor_information(
    ...
    def _assess_vendor_risk(
        self, vendor_name: str, invoice: InvoiceData, history_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class DateValidationAgent(SpecializedAgent):
    ...
    async def process(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Processing date validation for invoice {invoice.invoice_number}"
        )
        ...
        # Validate due date
        due_date_issues = self._validate_due_date(
    ...
    def _validate_due_date(
        self, due_date: str, invoice_date: str
    ) -> List[Dict[str, Any]]:
...
class TaxCalculationAgent(SpecializedAgent):
    ...
    async def process(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Processing tax calculation for invoice {invoice.invoice_number}"
        )
    ...
    def _check_tax_exemptions(self, invoice: InvoiceData) -> List[Dict[str, Any]]:
        ...
        if invoice.tax_amount == 0 and invoice.subtotal > 0:
            ...
            text_to_check = (
                invoice.notes
                + " "
                + " ".join(item.get("description", "") for item in invoice.line_items)
            ).lower()
            ...
            has_exemption_indicator = any(
                indicator in text_to_check for indicator in exemption_indicators
            )
                ...
                issues.append(
                    {
                        ...
                        "exemption_indicators": [
                            ind for ind in exemption_indicators if ind in text_to_check
                        ],
                    }
                )
...
class POMatchingAgent(SpecializedAgent):
    ...
    async def process(
        ...
        if not po_number:
            ...
            if po_match_results.get("po_found"):
                amount_validation = self._validate_amounts_against_po(
                    invoice, po_number
                )
    ...
    def _match_to_po_database(
        self, invoice: InvoiceData, po_number: str
    ) -> Dict[str, Any]:
        ...
        if po_number not in self.mock_po_database:
            ...
            if po_vendor != invoice_vendor:
                ...
                recommendations.append(
                    "Verify vendor information matches purchase order"
                )
            ...
            if po_data["status"] != "approved":
                ...
                recommendations.append(
                    "Ensure purchase order is approved before processing"
                )
    ...
    def _validate_amounts_against_po(
        self, invoice: InvoiceData, po_number: str
    ) -> Dict[str, Any]:
        ...
        percentage_difference = (
            (amount_difference / po_total) * 100 if po_total > 0 else 0
        )
    ...
    def _validate_items_against_po(
        ...
        if unmatched_items:
            ...
            recommendations.append(
                "Verify line items match purchase order specifications"
            )
...
class FraudDetectionAgent(SpecializedAgent):
    ...
    def _initialize_fraud_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "amount_just_under_limit": {
                "common_limits": [1000, 5000, 10000],
                "threshold": 50,
                "weight": 0.5,
            },
        }
    ...
    async def process(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Processing fraud detection for invoice {invoice.invoice_number}"
        )
        ...
        findings.append(
            {
                ...
                "severity": (
                    "critical"
                    if fraud_risk > 0.7
                    else "high" if fraud_risk > 0.4 else "low"
                ),
                ...
                "risk_level": (
                    "HIGH"
                    if fraud_risk > 0.7
                    else "MEDIUM" if fraud_risk > 0.4 else "LOW"
                ),
            }
        )
    ...
    async def _detect_fraud_patterns(self, invoice: InvoiceData) -> Dict[str, Any]:
        ...
        for limit in self.fraud_patterns["amount_just_under_limit"]["common_limits"]:
            ...
            if limit - threshold <= invoice.total_amount < limit:
                fraud_score += (
                    self.fraud_patterns["amount_just_under_limit"]["weight"] * 0.4
                )
                findings.append(
                    {
    ...
    async def _detect_statistical_anomalies(
        ...
        if vendor_history["recent_invoices"]:
            recent_amounts = [
                inv.get("total_amount", 0) for inv in vendor_history["recent_invoices"]
            ]

            if recent_amounts:
                avg_amount = sum(recent_amounts) / len(recent_amounts)
                std_dev = (
                    sum((x - avg_amount) ** 2 for x in recent_amounts)
                    / len(recent_amounts)
                ) ** 0.5
    ...
    def _analyze_digital_forensics(self, invoice: InvoiceData) -> Dict[str, Any]:
        ...
        for item in invoice.line_items:
            try:
                ...
                if total > 0:
                    decimal_places = (
                        len(str(total).split(".")[-1]) if "." in str(total) else 0
                    )
        ...
        if line_item_precisions:
            ...
            if (
                all(p == 0 for p in line_item_precisions)
                and len(line_item_precisions) > 1
            ):
                fraud_score += 0.2
                findings.append(
                    {
...
class ComplianceCheckAgent(SpecializedAgent):
    ...
    async def process(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Processing compliance check for invoice {invoice.invoice_number}"
        )
    ...
    def _check_tax_compliance(self, invoice: InvoiceData) -> Dict[str, Any]:
        ...
        if invoice.subtotal > 0:
            ...
            if tax_rate > 0.15:  # 15% seems high for most jurisdictions
                ...
                confidence_reduction += 0.1
                recommendations.append(
                    "Verify tax rate compliance with local regulations"
                )
    ...
    def _check_procurement_compliance(self, invoice: InvoiceData) -> Dict[str, Any]:
        ...
        if (
            invoice.total_amount >= procurement_rules["po_required_threshold"]
    ...
    def _check_aml_compliance(self, invoice: InvoiceData) -> Dict[str, Any]:
        ...
        if vendor_history["recent_invoices"]:
            recent_total = sum(
                ...
                for inv in vendor_history["recent_invoices"][
                    -10:
            ...
            if recent_total >= 25000:  # $25K in recent transactions
                ...
                recommendations.append(
                    "Review cumulative transaction patterns for AML compliance"
                )
...
class AnalyticsEngineAgent(SpecializedAgent):
    ...
    async def process(
        ...
        verification_results = (
            context.get("verification_results", []) if context else []
        )
        ...
        return self._create_result(
            "PASS", confidence, findings, recommendations, processing_time
        )
    ...
    def _generate_verification_summary(
        ...
        for result in verification_results:
            ...
            perf["avg_confidence"] = (
                perf["avg_confidence"] * (perf["total_processed"] - 1)
                + result.confidence
            ) / perf["total_processed"]
            perf["avg_processing_time"] = (
                perf["avg_processing_time"] * (perf["total_processed"] - 1)
                + result.processing_time
            ) / perf["total_processed"]

            status = result.status
            perf["status_distribution"][status] = (
                perf["status_distribution"].get(status, 0) + 1
            )
...
class InvoiceVerificationOrchestrator:
    ...
    async def process_invoice(self, invoice_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for agent_name, agent in self.agents.items():
            try:
                ...
                context = (
                    ...
                    if agent_name == "analytics_engine"
                    else None
                )
        ...
        final_status = self._determine_final_status(
            verification_results, critical_failures
        )
        ...
        self.logger.info(
            f"Completed verification for invoice {invoice.invoice_number}: {final_status}"
        )

        return result

    def _determine_final_status(
        self, results: List[VerificationResult], critical_failures: List[str]
    ) -> str:
