...
from data_models import JSONParser, DataValidator
...
class SummarizeSignature(dspy.Signature):
    ...
    summary: str = dspy.OutputField(desc="Respond with ONLY a JSON object containing fraud_risk, conclusion, and risk_factors")
...
class ErrorAnalysisSignature(dspy.Signature):
    ...
    failed_step: str = dspy.InputField(desc="Which step failed (planning, validation, execution, etc.)")
    
    analysis: str = dspy.OutputField(desc="JSON object with comprehensive error analysis and suggestions")
...
class FallbackPlanSignature(dspy.Signature):
    ...
    fallback_tasks: str = dspy.OutputField(desc="Respond with ONLY a comprehensive JSON array of tasks ensuring complete fraud coverage")
...
class ConfidenceSignature(dspy.Signature):
    ...
    confidence: str = dspy.OutputField(desc="JSON object with confidence assessment and factors")
...
def validate_signature_response(signature_name: str, response: str, expected_type: type = dict) -> bool:
...
def get_signature_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "signature_list": [
            "PlanSignature", "ValidateSignature", "SummarizeSignature",
            "ReviewSignature", "ErrorAnalysisSignature", "FallbackPlanSignature",
            "ConfidenceSignature"
        ],
...
def test_signature_response_parsing():
    ...
    for response, expected_type, should_pass in test_cases:
        try:
            if expected_type == list:
                parsed = extract_tasks_from_response(type('Response', (), {'tasks': response})())
                success = bool(parsed) == should_pass
            else:
                success = validate_signature_response("Test", response, expected_type) == should_pass
...
def process_llm_response_with_fallback(response: Any, signature_type: str, fallback_value: Any = None) -> Any:
    """Process LLM response with intelligent fallback handling"""
    try:
        if signature_type == "plan" or signature_type == "validate" or signature_type == "fallback":
...
def validate_fraud_risk_consistency(summary: Dict[str, Any], agent_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "issues": issues,
            "recommendations": [
                "Adjust fraud risk level to match findings" if issues else "Risk assessment is consistent",
                "Update risk factors to reflect agent results" if missing_factors else "Risk factors are complete"
            ]
        }
...
__all__ = [
    'PlanSignature', 'ValidateSignature', 'SummarizeSignature', 'ReviewSignature',
    'ErrorAnalysisSignature', 'FallbackPlanSignature', 'ConfidenceSignature',
    'DSPyModuleManager', 'module_manager',
    'create_dspy_modules', 'get_dspy_module', 'reinitialize_modules',
    'extract_tasks_from_response', 'extract_summary_from_response', 'extract_review_from_response',
    'extract_confidence_from_response', 'validate_signature_response',
    'process_llm_response_with_fallback', 'validate_fraud_risk_consistency',
    'get_signature_status', 'test_signature_response_parsing'
]