...
class AgentLogger:
    ...
    def _setup_loggers(self):
        ...
        for component in components:
            ...
            file_handler = logging.FileHandler(self.log_dir / f"{component}.log")
...
class BaseAgent:
    ...
    async def receive_message(self, message: AgentMessage) -> Optional[AgentMessage]:
        ...
        self.logger.debug(
            f"Received message: {message.message_type.value} from {message.sender}"
        )
    ...
    async def handle_status_update(self, message: AgentMessage):
        ...
        self.logger.info(
            f"Status update from {message.sender}: {message.content.get('status')}"
        )
...
class OrchestratorAgent(BaseAgent):
    ...
    async def validate_and_route_tool_call(
        ...
        if not self._validate_schema(tool_call):
            return self._create_error_response(message, "Invalid tool call schema")

        # Safety policy validation
        if not self._apply_safety_policies(tool_call):
            return self._create_error_response(message, "Safety policy violation")
        ...
        if tool_name == "validate_invoice":
            ...
            return self._create_error_response(message, f"Unknown tool: {tool_name}")
...
class LLMAgent(BaseAgent):
    ...
    def _generate_direct_response(self, request: Dict[str, Any]) -> str:
        ...
        if "help" in query.lower():
            return (
                "I can help you validate invoices, collect feedback on validation results, "
                "and generate analytics reports. What would you like me to do?"
            )
        else:
            return (
                "I understand your query, but I need more specific information. "
    ...
    def _generate_contextual_response(
        ...
        if intent == "validate_invoice":
            ...
            return "I've processed your request and here are the results: " + str(
                results
            )
    ...
    def _format_feedback_response(self, results: Dict[str, Any]) -> str:
        ...
        if results.get("status") == "success":
            return (
                "Thank you for your feedback! I've recorded your input and will use it to "
    ...
    def _format_analytics_response(self, results: Dict[str, Any]) -> str:
        ...
        if results.get("status") == "success":
            ...
            return (
                f"Here's your analytics summary:\n"
    ...
    async def _apply_self_correction(
        self, results: Dict[str, Any], context: Dict[str, Any], conversation_id: str
    ) -> Optional[str]:
        """
...
class ValidationAgent(BaseAgent):
    ...
    async def validate_invoice(self, message: AgentMessage) -> AgentMessage:
        ...
        try:
            ...
            await self._save_validation_results(
                invoice_data, discrepancies, conversation_id
            )
    ...
    def _calculate_confidence(
        self, discrepancies: List[Dict], invoice_data: Dict[str, Any]
    ) -> float:
    ...
    async def _save_validation_results(
        self, invoice_data: Dict, discrepancies: List, conversation_id: str
    ):
...
class FeedbackAgent(BaseAgent):
    ...
    def _load_feedback_history(self) -> List[Dict[str, Any]]:
        ...
        feedback_file = Path(
            "invoice_feedback_system/data/feedback/feedback_history.json"
        )
        try:
    ...
    def _analyze_feedback_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_action": (
                max(action_counts.keys(), key=action_counts.get)
                if action_counts
                else None
            ),
            "feedback_trend": (
                "positive"
                if action_counts.get("confirmed", 0) > action_counts.get("rejected", 0)
...
class AnalyticsAgent(BaseAgent):
    ...
    async def generate_analytics(self, message: AgentMessage) -> AgentMessage:
        ...
        try:
            ...
            result = {"status": "success", "analytics": analytics}
        ...
        if analytics_dir.exists():
            for file_path in analytics_dir.glob("validation_*.json"):
                try:
                    ...
                    self.logger.warning(
                        f"Could not load validation data from {file_path}: {e}"
                    )
    ...
    def _analyze_validations(self, validation_data: List[Dict]) -> Dict[str, Any]:
        ...
        total_discrepancies = sum(
            len(v.get("discrepancies", [])) for v in validation_data
        )
        ...
        return {
            ...
            "average_discrepancies_per_invoice": (
                round(total_discrepancies / total_validations, 2)
                if total_validations > 0
                else 0
            ),
            "common_discrepancy_types": discrepancy_types,
            "most_common_discrepancy": (
                max(discrepancy_types.keys(), key=discrepancy_types.get)
    ...
    def _calculate_satisfaction(self, action_counts: Dict[str, int]) -> str:
        ...
        positive_actions = action_counts.get("confirmed", 0) + action_counts.get(
            "helpful", 0
        )
        negative_actions = action_counts.get("rejected", 0) + action_counts.get(
            "incorrect", 0
        )
    ...
    def _calculate_system_performance(
        self, validation_data: List[Dict], feedback_data: List[Dict]
    ) -> Dict[str, Any]:
        """Calculate overall system performance metrics"""
        # Calculate accuracy based on feedback
        confirmed_validations = len(
            [f for f in feedback_data if f.get("user_action") == "confirmed"]
        )
        total_validations_with_feedback = len(feedback_data)

        accuracy = (
            confirmed_validations / total_validations_with_feedback
            if total_validations_with_feedback > 0
            else 0
        )
        ...
        return {
            ...
            "feedback_rate": (
                round(total_validations_with_feedback / len(validation_data), 3)
                if validation_data
                else 0
            ),
    ...
    def _generate_recommendations(
        ...
        if validation_data:
            ...
            if (
                total_discrepancies > len(validation_data) * 0.5
            ):  # More than 50% have issues
                recommendations.append(
                    "Consider reviewing validation rules to reduce false positives"
                )
        ...
        if feedback_data:
            negative_feedback = len(
                [
                    ...
                    if f.get("user_action") in ["rejected", "incorrect"]
                ]
            )
            if negative_feedback > len(feedback_data) * 0.3:  # More than 30% negative
                recommendations.append(
                    "System accuracy needs improvement - consider model retraining"
                )
    ...
    async def _save_analytics_report(
        self, analytics: Dict[str, Any], conversation_id: str
    ):
        """Save analytics report"""
...
class InvoiceFeedbackSystem:
    ...
    async def process_user_query(self, query: str) -> str:
        ...
        self.system_logger.info(
            f"Processing user query [conversation: {conversation_id}]: {query[:100]}..."
        )
