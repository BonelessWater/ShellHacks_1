...
class DataMonitor:
    ...
    def monitor_data_quality(
        ...
        # Some datasets may not have exact row duplicates but have repeated values
        # in key columns (like amounts). If no row-level duplicates are found,
        # inspect column-level duplicated values and surface them as issues.
        if quality_metrics["duplicate_rows"] == 0:
            for col in df.columns:
                try:
                    ...
                    dup_count = 0
                if dup_count > 0:
                    quality_metrics["duplicate_rows"] = int(dup_count)
                    quality_metrics["quality_issues"].append(
                        f"Column {col} has {dup_count} repeated values"
                    )
                    break
    ...
    def _log_monitoring_event(self, event_type: str, event_data: Dict[str, Any]):
        ...
        # Convert numpy/int64/etc to native Python types for JSON serialization
        ...
        safe_data = _make_jsonable(event_data)
