...
def test_all_keys_systematically() -> dict:
    ...
    if not keys:
        log.error(
            "❌ No API keys found. Add GOOGLE_API_KEY and/or GOOGLE_API_KEY_0..9 to your .env."
        )
    ...
    for idx, key in enumerate(keys):
        ...
        try:
            ...
            if detailed_result["success"]:
                ...
                log.info(
                    f"   🤖 Response: {detailed_result['generation_response'][:100]}{'...' if len(detailed_result['generation_response']) > 100 else ''}"
                )
            else:
                log.error(
                    f"❌ {key_name} FAILED - {detailed_result['error_type']}: {detailed_result['error_message']}"
                )
                for rec in detailed_result["recommendations"]:
                    log.warning(f"   💡 {rec}")
        except Exception as e:
