...
class InvoiceFraudDetector:
    ...
    def _get_api_key(self) -> str:
        ...
        raise ValueError(
            "No valid Google API key found. Set GOOGLE_API_KEY in .env file"
        )
    ...
    def analyze_invoice(self, invoice_data: str) -> Dict[str, Any]:
        ...
        log.info(
            f"📋 Summoning {len(agents_to_summon)} specialist agents: {', '.join(agents_to_summon)}"
        )
        ...
        for agent_type in agents_to_summon:
            ...
            log.info(
                f"✅ {agent_type}: Risk {response.risk_score}/10, Confidence {response.confidence}/10"
            )
...
"""
        ...
        try:
            ...
            if "[" in agents_text and "]" in agents_text:
                ...
                valid_agents = [
                    a for a in selected_agents if a in self.available_agents
                ]
                return (
                    valid_agents
                    if valid_agents
    ...
    def _run_specialist_agent(
        self, agent_type: str, invoice_data: str
    ) -> AgentResponse:
...
"""
    ...
    def _compile_results(
        self, invoice_data: str, agent_responses: List[AgentResponse]
    ) -> Dict[str, Any]:
        ...
        avg_confidence = (
            total_confidence / len(agent_responses) if agent_responses else 5
        )
...
"""
        ...
        try:
            ...
            next_steps = self._extract_field(
                text, "NEXT_STEPS", "Manual review recommended"
            )

            top_concerns = []
            if top_concerns_text.lower() != "none":
                top_concerns = [
                    concern.strip() for concern in top_concerns_text.split(",")
                ]
